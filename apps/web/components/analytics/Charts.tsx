...
export function DonutChart({ segments, total }: DonutChartProps) {
  ...
  return (
    <svg viewBox="0 0 200 140" className="w-full h-full overflow-visible">
      ...
      {/* Center label */}
      <text x={CX} y={CY - 6} textAnchor="middle" fill="white" fontSize={18} fontWeight={800}>
        {total}
      </text>
      <text
        x={CX}
      ...
      {segments.map((seg, i) => (
        <g key={seg.label} transform={`translate(140, ${16 + i * 22})`}>
          <rect
            ...
            rx={2}
            fill={seg.color}
            opacity={seg.count === 0 ? 0.25 : 0.85}
          />
          <text
            x={12}
            y={8}
            fill={hovered === seg.label ? 'white' : '#9CA3AF'}
            fontSize={9}
            fontWeight={700}
          >
            {seg.label}
          </text>
          <text x={58} y={8} fill="white" fontSize={9} fontWeight={800} textAnchor="end">
            {seg.count}
          </text>
        </g>
      ))}
