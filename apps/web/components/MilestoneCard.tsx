...
export default function MilestoneCard({
  ...
  const handleApprove = () => {
    ...
    console.log(
      `approve milestone ${milestoneIndex} on escrow #${escrowId} — not implemented, see issue #M3`
    );
  };

  return (
    <div
      className={`glass p-6 rounded-2xl border transition-all duration-300 ${isReleased ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-white/5'}`}
    >
      ...
      <div className="flex items-center gap-1.5 text-xs text-dark-600 font-semibold mb-4">
        <Users size={14} />
        <span>
          Approvals: {milestone.approvalCount} of {approvers.length} required
        </span>
      </div>
