...
import {
  Shield, Play, Pause, Settings, DollarSign, Database,
  Activity, ArrowLeft, RefreshCw, Key, Users, CheckCircle,
  AlertTriangle, Check, Clipboard, Trash, Plus
} from 'lucide-react';
...
export default function AdminPage() {
  ...
  const [feeRecipient, setFeeRecipient] = useState('GB5S2KJS2KXZT4V6X73D7W6UQLW5Z5XQ7PBYQ2Q7O6D3D5U4A6M5Z5Z5Z');
  ...
  const handleTogglePause = () => {
    ...
    const toastId = toast.loading(targetState ? "Pausing all contract operations..." : "Resuming all contract operations...");

    setTimeout(() => {
      setIsPaused(targetState);
      setLoading(false);
      toast.success(targetState ? "System paused successfully (Circuit Breaker active)." : "System resumed successfully.", { id: toastId });

      // Add to audit logs
      const log: AuditLog = {
        id: Date.now().toString(),
        action: targetState ? 'Emergency Pause Activated' : 'Emergency Pause Deactivated (System Resume)',
        actor: publicKey ? `${publicKey.slice(0, 4)}...${publicKey.slice(-4)}` : 'Admin (FREIGHTER)',
  ...
  const handleUpdateRegistry = (key: keyof typeof registries) => {
    ...
    if (!isValidContract(val)) {
      toast.error(`Invalid ${key} contract address. Must be a valid C... address of 56 characters.`);
      return;
    }
  ...
  return (
    <div className="min-h-[calc(100vh-160px)] bg-dark-900 relative overflow-hidden">
      ...
      <main className="max-w-6xl w-full mx-auto px-6 py-12 relative z-10 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="space-y-1">
            ...
            <p className="text-sm text-dark-500">
              Manage core system contract addresses, emergency pauses, platform fees, and governance rules.
            </p>
          </div>
            ...
            >
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-rose-300/80 leading-relaxed">
                  All stream withdrawals, milestones approvals, and deployment actions have been frozen at the factory contract level. Users cannot initiate new transactions until the contract is resumed.
                </p>
              </div>
            </motion.div>
          )}
          ...
          <div className="lg:col-span-2 space-y-8">
                  ...
                  <div key={key} className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        onChange={(e) => setRegistryInputs(r => ({ ...r, [key]: e.target.value }))}
              ...
              <form onSubmit={handleUpdateFees} className="space-y-4">
                <div className="grid sm:grid-cols-3 gap-4">
                  <div className="space-y-1.5 sm:col-span-1">
                    <label className="text-xs text-dark-400 font-bold uppercase tracking-wider">Fee Percentage</label>
                    <div className="relative">
                      ...
                      />
                      <span className="absolute right-3 top-2.5 text-xs font-bold text-dark-500">%</span>
                    </div>
                  </div>
                  <div className="space-y-1.5 sm:col-span-2">
                    <label className="text-xs text-dark-400 font-bold uppercase tracking-wider">Fee Collector Address</label>
                    <input
                      type="text"
                      value={feeRecipient}
            ...
            <div className="glass p-6 rounded-2xl border border-white/5 space-y-4">
              ...
              <div className="space-y-3 max-h-[250px] overflow-y-auto pr-1">
                {auditLogs.map((log) => (
                  <div key={log.id} className="bg-dark-900/60 p-3.5 rounded-xl border border-white/5 flex justify-between items-center gap-4 text-xs">
                    <div className="space-y-1">
            ...
            <div className="glass p-6 rounded-2xl border border-white/5 space-y-4">
              ...
              <div className="space-y-3 text-xs">
                <div className="flex justify-between py-2 border-b border-white/5">
                  ...
                  <span className={`font-bold flex items-center gap-1.5 ${isPaused ? 'text-rose-400' : 'text-emerald-400'}`}>
                    <span className={`h-2 w-2 rounded-full ${isPaused ? 'bg-rose-500 animate-pulse' : 'bg-emerald-500'}`} />
              ...
              <div className="space-y-3.5">
                ...
                <div className="space-y-1.5">
                  <div className="flex justify-between text-xs">
                    <span className="text-dark-400 font-bold uppercase tracking-wider">Required Approvals</span>
                    <span className="font-bold text-white font-mono">{threshold}-of-{signers.length}</span>
                  </div>
                  <input
                    type="range"
                    min="1"
                ...
                <div className="space-y-2">
                  <span className="text-[10px] text-dark-500 font-bold uppercase tracking-wider block">Admin Signers</span>
                  <div className="space-y-2 max-h-[220px] overflow-y-auto pr-1">
                    {signers.map((s, idx) => (
                      <div key={idx} className="bg-dark-900/60 p-2.5 rounded-xl border border-white/5 flex items-center justify-between gap-3">
                ...
                <form onSubmit={handleAddSigner} className="space-y-2 pt-2 border-t border-white/5">
                  <span className="text-[10px] text-dark-500 font-bold uppercase tracking-wider block">Propose New Signer</span>
                  <div className="flex gap-2">
                    <input
                      type="text"
                      value={newSigner}
        ...
        {/* Back Link */}
        <div className="pt-4">
          <Link href="/" className="inline-flex items-center gap-1.5 text-xs text-dark-600 hover:text-white transition font-bold">
