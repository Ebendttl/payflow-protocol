...
import { BarChart3, TrendingUp, Activity, Wallet, RefreshCw, FlaskConical, ArrowUpRight, ChevronUp, ChevronDown, Minus } from 'lucide-react';
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <span className="text-[10px] font-bold text-dark-500 uppercase tracking-widest">{label}</span>
        <div className="h-8 w-8 rounded-xl flex items-center justify-center" style={{ background: `${accent}18`, border: `1px solid ${accent}30` }}>
...
function StreamsTable({ streams, nowSec }: { streams: Stream[]; nowSec: number }) {
  ...
  const sorted = useMemo(() => [...streams].sort((a, b) => {
    let av: number, bv: number;
    if (sort === 'id')            { av = Number(a.id); bv = Number(b.id); }
    else if (sort === 'totalAmount')  { av = Number(a.totalAmount); bv = Number(b.totalAmount); }
    else if (sort === 'claimedAmount'){ av = Number(a.claimedAmount); bv = Number(b.claimedAmount); }
    else { av = a.status.charCodeAt(0); bv = b.status.charCodeAt(0); }
    return dir === 'desc' ? bv - av : av - bv;
  }), [streams, sort, dir]);

  const SortIcon = ({ k }: { k: SortKey }) =>
    sort === k
      ? dir === 'desc' ? <ChevronDown size={11} className="text-primary-light" /> : <ChevronUp size={11} className="text-primary-light" />
      ...
      <table className="w-full text-xs">
        <thead>
          <tr className="border-b border-white/5 bg-dark-800/50">
            {([['id','#ID'],['totalAmount','Total'],['claimedAmount','Claimed'],['status','Status']] as [SortKey, string][]).map(([k, lbl]) => (
              <th key={k} onClick={() => toggle(k)}
                ...
                <span className="flex items-center gap-1">{lbl} <SortIcon k={k} /></span>
              </th>
            ))}
            <th className="text-left px-4 py-3 text-dark-500 font-bold uppercase tracking-widest whitespace-nowrap">Recipient</th>
            <th className="text-left px-4 py-3 text-dark-500 font-bold uppercase tracking-widest whitespace-nowrap">Progress</th>
          </tr>
        </thead>
        <tbody>
          {sorted.map((s, i) => {
            ...
            return (
              ...
              >
                ...
                <td className="px-4 py-3 font-mono font-bold text-white">
                  {(Number(s.totalAmount)/1e7).toFixed(2)}<span className="ml-1 text-dark-500 text-[10px]">{s.token.slice(0,6)}</span>
                </td>
                <td className="px-4 py-3 font-mono text-dark-300">
                  {(Number(s.claimedAmount)/1e7).toFixed(2)}
                </td>
                <td className="px-4 py-3">
                  <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold uppercase border ${statusStyle[s.status]}`}>
                    {s.status}
                  </span>
                </td>
                ...
                <td className="px-4 py-3 min-w-[110px]">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-dark-400 font-bold w-8 text-right">{pct}%</span>
                  </div>
                </td>
              </motion.tr>
            );
    ...
    <div className="min-h-[calc(100vh-160px)] bg-dark-900 relative overflow-hidden">
      ...
      <main className="max-w-7xl w-full mx-auto px-6 py-12 space-y-8 relative z-10">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-dark-500">Your real on-chain streams will populate all charts below.</p>
              </div>
            </div>
            <div className="flex gap-2 shrink-0">
              <WalletOptionButton label="Freighter" onClick={() => connect('freighter')} isConnecting={isConnecting && walletType === 'freighter'} />
              <WalletOptionButton label="LOBSTR"    onClick={() => connect('lobstr')}    isConnecting={isConnecting && walletType === 'lobstr'}    />
            </div>
          </motion.div>
        )}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
          <StatCard delay={0}    label="Total Value Locked"  value={`$${summary.tvl.toLocaleString(undefined,{maximumFractionDigits:2})}`}       sub="active + paused streams"   icon={Wallet}    accent="#0D9488" />
          <StatCard delay={0.06} label="Total Claimed"       value={`$${summary.totalClaimed.toLocaleString(undefined,{maximumFractionDigits:2})}`} sub="all-time disbursed"        icon={TrendingUp} accent="#8B5CF6" />
          <StatCard delay={0.12} label="Claimable Now"       value={`$${summary.claimable.toFixed(4)}`}                                             sub="accrued − claimed, live"   icon={ArrowUpRight} accent="#10B981" />
          <StatCard delay={0.18} label="Active Streams"      value={String(summary.activeCount)}                                                     sub={`${(summary.ratePerSec * 86400).toFixed(2)} tokens/day`} icon={Activity} accent="#F59E0B" />
        </div>
        ...
        >
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-[11px] text-dark-500">Cumulative earnings from all active streams — next 30 days</p>
            </div>
          ...
          {streams.length > 0
            ...
            : (
              <div className="glass p-10 rounded-2xl border border-white/5 text-center">
                ...
                <p className="text-xs text-dark-500 mt-1">Create your first stream to see analytics here.</p>
              </div>
            )
          }
        </motion.div>
