...
export function printStream(stream: Stream): void {
  ...
  console.log(
    chalk.dim('  Amount    : ') + chalk.white(`${Number(stream.totalAmount) / 1e7} (total)`)
  );
...
export function printEscrow(escrow: Escrow): void {
  ...
  console.log(
    chalk.dim('  Threshold  : ') +
      chalk.white(`${escrow.threshold} of ${escrow.approvers.length} approvers`)
  );
  ...
  escrow.milestones.forEach((m, i) => {
    const mColor =
      m.status === 'Released' ? chalk.green : m.status === 'Approved' ? chalk.yellow : chalk.dim;
